...
<footer class="site-footer">
    <div class="container footer-grid">
        <div class="footer-brand"><img src="<?= h(asset('brand/skillspark-wordmark-light.svg')) ?>" width="190" height="48" alt="SkillsPark Tech Hub"><p><?= h($site['primary_message']) ?></p><ul class="footer-socials" aria-label="Follow SkillsPark"><?php foreach ($contact['socials'] as $social): $socialUrl = safe_external_url($social['url']); $socialIcon = $socialIcons[$social['label']] ?? null; if ($socialUrl && $socialIcon): ?><li><a href="<?= h($socialUrl) ?>" target="_blank" rel="noopener noreferrer" aria-label="<?= h($social['label']) ?>" title="<?= h($social['label']) ?>"><?= icon($socialIcon) ?><span class="sr-only"><?= h($social['label']) ?></span></a></li><?php endif; endforeach; ?></ul></div>
